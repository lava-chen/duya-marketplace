---
name: zotero
description: Use Zotero Desktop from duya to enable/probe the local API, search a local Zotero library, list items/collections/tags, export BibTeX, insert citation keys into LaTeX or Markdown drafts, read indexed full text when requested, and import BibTeX/RIS records into Zotero through the connector server. Use when the user mentions Zotero, citations, references.bib, BibTeX export, local Zotero API, localhost:23119, or adding citations from a Zotero library.
allowed-tools: Bash, Read, Write, Edit, Glob, Grep
---

# Zotero

Use this skill to operate a user's local Zotero Desktop library from duya.

## Duya capability binding

This skill is bound to duya's own capabilities. The Zotero local API and
connector server are plain HTTP surfaces on `http://127.0.0.1:23119`, so all
interaction goes through the bundled stdlib-only Python helper. No external
MCP server or SDK is required.

The core helper lives next to this `SKILL.md`:

```bash
python3 ./scripts/zotero.py <command>
```

The helper is stdlib-only (`urllib`, `configparser`, `subprocess`) and runs with
Python 3. It does not require any third-party packages.
...
```bash
python3 ./scripts/zotero.py cite --query "Attention Is All You Need" --tex paper.tex --bib references.bib --marker '<cite>'
```
...
python3 ./scripts/zotero.py status --json
python3 ./scripts/zotero.py probe --json

# Library inventory
python3 ./scripts/zotero.py inventory
python3 ./scripts/zotero.py collections
python3 ./scripts/zotero.py tags

# Search and export
python3 ./scripts/zotero.py search "BERT"
python3 ./scripts/zotero.py export-bibtex --out references.bib
python3 ./scripts/zotero.py export-bibtex --item-key PXW99EKT
python3 ./scripts/zotero.py citations --style apa --json

# Draft editing
python3 ./scripts/zotero.py cite --item-key PXW99EKT --tex paper.tex --bib references.bib --marker '<cite>'
python3 ./scripts/zotero.py cite --query "BERT" --markdown notes.md --bib references.bib --marker '<cite>'

# Attachments and full text; use only on request
python3 ./scripts/zotero.py children PXW99EKT --json
python3 ./scripts/zotero.py fulltext 2JAZS9U8 --out attention-fulltext.txt
python3 ./scripts/zotero.py file-url 2JAZS9U8

# Writes to Zotero; confirm first unless explicitly requested
python3 ./scripts/zotero.py selected-target --json
python3 ./scripts/zotero.py import-bibtex --file new-reference.bib --yes
python3 ./scripts/zotero.py import-ris --file new-reference.ris --yes
```
